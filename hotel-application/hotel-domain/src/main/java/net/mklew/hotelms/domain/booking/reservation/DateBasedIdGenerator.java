...
/**
 * DateBasedIdGenerator creates business friendly ids based on unique ids from for e.g. database sequence
 * Ids have at least 8 digits and are quite distinct even when generated from sequence.
 */
public class DateBasedIdGenerator  implements IdGenerator
{
    private final DateTime dateTime;

    public static DateBasedIdGenerator newGenerator()
    {
        return new DateBasedIdGenerator(DateTime.now());
    }

    public DateBasedIdGenerator(DateTime dateTime)
    {
    ...
    public static void main(String [] args)    throws Exception
    {
        for(int i = 0 ; i < 10 ; i++)
        {
            DateBasedIdGenerator generatorDateBased = newGenerator();

            System.out.println(generatorDateBased.generateId(i));
